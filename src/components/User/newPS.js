...
export default class PersonalService extends Component {
  ...
  componentWillMount() {
    ...
    this.setState({
      ...
      filterSoplush : this.props.navigation.getParam('service'),
      filterplush : this.props.navigation.getParam('service'),
      img: this.props.navigation.getParam('image')
    })
  }
  ...
  workFunction(val, type, ind) {
    let { plush, cart, amount, plushData, filterplush, soplushData, filterSoplush   } = this.state;
    ...
    if (val.p_checked != 0) {
      ...
      this.setState({ cart, amount: amount, filterplush, filterSoplush, plushData, soplushData });
    } else {
  ...
  FilterBeautician = () => {
    ...
    const { faltu, filterSoplush, filterplush, soplushData, plushData } = this.state
  ...
  searchFilterFunction = text => {
    ...
    if (text !== '') {
      ...
      this.setState({ faltu: !faltu, filterSoplush, filterplush });
      console.log('filterSoplush', filterSoplush);
    } else {
      filterplush = plushData;
      filterSoplush = soplushData;
      this.setState({ faltu: !faltu, filterplush, filterSoplush });
    }
    this.setState({ text: text });
  };

  render() {
    const {soplushData, filterSoplush, filterplush, plushData, img} = this.state
    ...
    // console.log('CART', filterplush, filterSoplush, img, this.props);
                                  ...
                                  <View
                                    ...
                                    <View
                                      ...
                                      <TouchableOpacity
                                        ...
                                        {value.p_checked === true && <Icon size={15} name="check" color="pink" />}
                                      </TouchableOpacity>
                                    ...
                                    <View
                                      ...
                                      <View
                                        ...
                                        <TouchableOpacity
                                          ...
                                          {value.s_checked === true && <Icon size={15} name="check" color="pink" />}
                                        </TouchableOpacity>
