...
export default class PassChange extends Component {
    ...
    checkField = (key) => {
        if (key == "currPassword") {
            if (this.state.currentPass.length < 1) {
                this.setState({ currPassErr: "Old Password is required" })
            } else {
    ...
    changePassword = async () => {
        ...
        if  (currentPass < 1) {
            this.setState({currPassErr: "Old Password is required", loader: false})
        }
        if (newPass < 1) {
        ...
        if (allCheck === true) {
            ...
            fetch("https://hnhtechsolutions.com/hassan/soplush/auth/change_password.php?action=change_password", {
                method: 'POST',
                // dataType: "json",
                headers: {
